...
def temp_git_repo(tmp_path: Path) -> Path:
    ...
    subprocess.run(["git", "checkout", "-b", "main"], cwd=tmp_path, check=False, capture_output=True)
    subprocess.run(["git", "branch", "-m", "main"], cwd=tmp_path, check=False, capture_output=True)
    return tmp_path
