    ...
    async def run_task(
        ...
        if not await self.healthcheck():
            if self._settings.environment in ("staging", "prod"):
                raise ConfigurationError("Antigravity CLI not found — mock adapter fallback blocked in staging/prod")
        ...
        try:
            ...
            return CodingResult(success=False, summary="Antigravity execution timed out", error="30 minute timeout exceeded")
        ...
        return post_run_metrics(repo, branch, success=True, summary=f"Antigravity completed on {branch}")
