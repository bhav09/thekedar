    ...
    async def run_task(
        ...
        if not await self.healthcheck():
            if self._settings.environment in ("staging", "prod"):
                raise ConfigurationError("Cursor CLI not found — mock adapter fallback blocked in staging/prod")
        ...
        if not command_available("cursor-agent"):
            # Ensure --cwd is included or cwd kwarg handles it
        ...
        try:
            ...
            return CodingResult(success=False, summary="Cursor execution timed out", error="30 minute timeout exceeded")
        ...
        return post_run_metrics(repo, branch, success=True, summary=f"Cursor completed task on {branch}")
